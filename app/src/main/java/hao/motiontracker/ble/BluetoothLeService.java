...
public class BluetoothLeService extends Service {
	...
	private int mNumActiveConnections = 0;
	...
	private BluetoothGattCallback mGattCallbacks = new BluetoothGattCallback() {
		...
		public void onCharacteristicChanged(BluetoothGatt gatt,
			...
			BluetoothDevice device = gatt.getDevice();
			broadcastUpdate(ACTION_DATA_NOTIFY, device.getAddress(),
					characteristic, BluetoothGatt.GATT_SUCCESS);
		}
		...
		public void onCharacteristicRead(BluetoothGatt gatt,
			...
			BluetoothDevice device = gatt.getDevice();
			broadcastUpdate(ACTION_DATA_READ, device.getAddress(),
		...
		public void onCharacteristicWrite(BluetoothGatt gatt,
				BluetoothGattCharacteristic characteristic, int status) {
			BluetoothDevice device = gatt.getDevice();
			broadcastUpdate(ACTION_DATA_WRITE, device.getAddress(),
					characteristic, status);
		}
	...
	public void onDestroy() {
		...
		for (int i = 0; i < mBluetoothGattList.size(); i++) {
			if (mBluetoothGattList.get(i) != null) {
	...
	public void readCharacteristic(BluetoothGattCharacteristic characteristic,
		...
		if (!checkGatt(address))
			return;
		mBusy = true;
		int index = mBluetoothDeviceAddressList.indexOf(address);
		mBluetoothGattList.get(index).readCharacteristic(characteristic);
	}
	...
	public boolean writeCharacteristic(
		...
		mBusy = true;
		int index = mBluetoothDeviceAddressList.indexOf(address);
		return mBluetoothGattList.get(index)
	...
	public boolean writeCharacteristic(
			BluetoothGattCharacteristic characteristic, String address) {
		if (!checkGatt(address))
			return false;
	...
	public int getNumServices(String address) {
		if (mBluetoothDeviceAddressList.contains(address)) {
			int index = mBluetoothDeviceAddressList.indexOf(address);
			return mBluetoothGattList.get(index).getServices().size();
		}
		return 0;
	}
	...
	public boolean setCharacteristicNotification(
		...
		int index = mBluetoothDeviceAddressList.indexOf(address);
		if (!mBluetoothGattList.get(index).setCharacteristicNotification(
		...
		mBusy = true;
		return mBluetoothGattList.get(index).writeDescriptor(clientConfig);
	}
	...
	public boolean connect(final String address) {
		...
		if (connectionState == BluetoothProfile.STATE_DISCONNECTED) {
			...
			if (mBluetoothDeviceAddressList.contains(address)) {
				int index = mBluetoothDeviceAddressList.indexOf(address);
				Log.d(TAG, "Re-use GATT connection " + index);
				if (mBluetoothGattList.get(index).connect()) {
					Log.d(TAG, "GATT re-connect successful.");
					return true;
				} else {
			...
			if (device == null) {
				...
				return false;
			} else if (mNumActiveConnections > MAX_CONNECTIONS) {
	...
	public void disconnect(String address) {
		...
		if (mBluetoothDeviceAddressList.contains(address)) {
			...
			Log.w(TAG, "Attempt to disconnect wrong address: " + address);
		}
	}
