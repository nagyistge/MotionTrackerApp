...
import hao.motiontracker.utils.Point3D;
 ...
 */
public enum Sensor {
	ACCELEROMETER(SensorTag.UUID_ACC_SERV, SensorTag.UUID_ACC_DATA, SensorTag.UUID_ACC_CONF) {
		@Override
	...
	GYROSCOPE(SensorTag.UUID_GYR_SERV, SensorTag.UUID_GYR_DATA, SensorTag.UUID_GYR_CONF, (byte) 7) {
		@Override
		public Point3D convert(final byte[] value) {
	...
	EULER_ANGLE(SensorTag.UUID_EUL_SERV, SensorTag.UUID_EUL_DATA, SensorTag.UUID_EUL_CONF) {
		@Override
		public Point3D convert(final byte[] value) {
			if (value.length < 12) {
