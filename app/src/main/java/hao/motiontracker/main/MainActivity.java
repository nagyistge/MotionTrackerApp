...
import hao.motiontracker.ble.BleDeviceInfo;
import hao.motiontracker.ble.BluetoothLeService;
import hao.motiontracker.utils.CustomToast;
...
public class MainActivity extends ViewPagerActivity {
	...
	private int mNumDeviceConnected = NO_DEVICE;
...
//				R.layout.fragment_help, R.id.webpage), "Help");
	...
	public void onBtnData(View view) {
		startDeviceActivity();
	}

	void onConnect(int pos) {
		BluetoothDevice mBluetoothDevice = mDeviceInfoList.get(pos).getBluetoothDevice();
		if (mNumDevs > 0) {
			int connState = mBluetoothManager.getConnectionState(
					mBluetoothDevice, BluetoothGatt.GATT);
			Log.d("haodebug", "connState is " + connState + " for address "
			...
			case BluetoothGatt.STATE_CONNECTED:
				...
				Log.d("haodebug",
						"Disconnecting " + mBluetoothDevice.getAddress());
				if (mNumDeviceConnected > NO_DEVICE) {
			...
			case BluetoothGatt.STATE_DISCONNECTED:
				...
				Log.d("haodebug", "Connecting " + mBluetoothDevice.getAddress());
	...
	public void updateGuiState() {
		...
		if (mBtEnabled) {
			if (mScanning) {
				// BLE Host connected
				if (mNumDeviceConnected > NO_DEVICE) {
					String txt = mNumDeviceConnected + " devices connected";
	...
	private boolean checkDeviceFilter(BluetoothDevice device) {
		...
		if (n > 0) {
			...
			for (int i = 0; i < n && !found; i++) {
				...
				if (name != null)
					found = name.equals(mDeviceFilter[i]);
			}
			return found;
		} else
	...
	private BleDeviceInfo findDeviceInfoByAddress(String address) {
		for (int i = 0; i < mDeviceInfoList.size(); i++) {
			if (mDeviceInfoList.get(i).getBluetoothDevice().getAddress()
					.equals(address)) {
				return mDeviceInfoList.get(i);
...
//			}
	...
	private BroadcastReceiver mReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			BluetoothDevice mBluetoothDevice = findDeviceInfoByAddress(address)
					.getBluetoothDevice();
